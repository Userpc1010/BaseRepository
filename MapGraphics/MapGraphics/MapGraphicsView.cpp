...
void MapGraphicsView::handleChildMouseDoubleClick(QMouseEvent *event)
{
    ...
    this->mapClicked_Left(scenePos);

    if (!lock_1) { Marker1 = new CircleObject( 16, true); }

    lock_1 = true;

    Marker1->drawMarkers( false, true, left_click, "", false);

    Marker1->setPos(scenePos);

     _scene->addObject(Marker1);
    }

    if ( event->button() == Qt::MidButton) {

    _scene->removeAllObject(); sceneLastPos.setX(0); sceneLastPos.setY(0); lock_1 = false; lock_2 = false; counter= 0; this->remove_all();

    }
}

void MapGraphicsView::find_marker(QPointF point)
{
  Find_marker = new CircleObject( 16, true);

  Find_marker->drawMarkers(false, true, Find_markerr, "", false);

  Find_marker->setPos(point);

  _scene ->addObject(Find_marker);

  centerOn(point);
}

void MapGraphicsView::plane_marker(QPointF point)
{
  Plane_marker->drawMarkers(false, true, Plane_markerr, "", true);

  Plane_marker->setPos(point);

  _scene ->addObject(Plane_marker);
...
{   
   ...
   if (lock_1) {Marker1->deleteLater(); lock_1 = false; }

   if (lock_2) {Marker->deleteLater(); Line->deleteLater(); sceneLastPos = sceneLastLastPos; lock_2 = false; this->remove(); counter--;}
...
void MapGraphicsView::handleChildMouseRelease(QMouseEvent *event)
{
    ...
    Line = new LineObject (scenePos ,sceneLastPos, 1 );

    sceneLastLastPos = sceneLastPos;

    sceneLastPos = scenePos;

    _scene->addObject(Line);

    Marker = new CircleObject( 16 ,true);

    Marker->drawMarkers (true, true, right_click, QString("%1").arg(counter), false);

    _scene->addObject(Marker);

    Marker->setPos(scenePos);

    this->mapClicked_Right(scenePos);
