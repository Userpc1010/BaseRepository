...
const quint32 DEFAULT_CACHE_MONTHS = 1;
const quint64 MAX_DISK_CACHE_READ_ATTEMPTS = 1000;
...
QImage *MapTileSource::fromDiskCache(const QString &cacheID)
{
    ...
    if (QDateTime::currentDateTimeUtc().secsTo(expireTime) <= 0)
    {
       ...
       if (!QFile::remove(path))
        ...
        else qDebug()<<"Remove old cache file" << path;
    ...
    {
        qDebug() <<"File found: "<<QDateTime::currentDateTimeUtc().secsTo(expireTime);
...
QDateTime MapTileSource::getTileExpirationTime(const QString &cacheID)
{
    ...
    {
        qWarning() << "Tile" << cacheID << "has unknown expire time. Resetting to default of" << DEFAULT_CACHE_MONTHS << "months.";
        expireTime = QDateTime::currentDateTimeUtc().addMonths(DEFAULT_CACHE_MONTHS);
...
void MapTileSource::setTileExpirationTime(const QString &cacheID, QDateTime expireTime)
{
    ...
    if (expireTime.isNull() || QDateTime::currentDateTimeUtc().secsTo(expireTime) <= 0)
    //На всякий случай добавил проверку на истечение срока давности
