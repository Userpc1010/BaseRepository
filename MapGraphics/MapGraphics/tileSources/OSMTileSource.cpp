...
{
    qDebug() << this << this->name() << "Закрытие";
}
...
QString OSMTileSource::name() const
{
    ...
    {
    case OSMTiles:
        return "Спутниковые снимки Esri";
        break;

    default:
        return "Неизвестные снимки";
        break;
    }
}
...
void OSMTileSource::fetchTile(quint32 x, quint32 y, quint8 z)
{
    ...
    if (_tileType == OSMTiles)
    {
        ...
        url = "/arcgis/rest/services/World_Imagery/MapServer/tile/%1/%3/%2";
    }
...
void OSMTileSource::handleNetworkRequestFinished()
{
    ...
    */
    ...
    if (reply->hasRawHeader("Cache-Control"))
    {
     ...
     Хз зачем был нужен этот расчёт если мы задаём время жизни плитки 1 месяц?
     */
